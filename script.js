const bun = document.querySelector(".bun");
const patty = document.querySelector(".patty");
const vegetables = document.querySelector(".vegetables");
const cheese = document.querySelector(".cheese");
const sauce = document.querySelector(".sauce");
const images = document.querySelectorAll(".images");
const square = document.querySelectorAll(".square");
const bunSquare = document.querySelector("#bun");
const pattySquare = document.querySelector("#patty");
const vegetablesSquare = document.querySelector("#vegetables");
const cheeseSquare = document.querySelector("#cheese");
const sauceSquare = document.querySelector("#sauce");
const topBun = document.querySelector(".top-bun");
const bottomBun = document.querySelector(".bottom-bun");
const addWhiteBuns = document.querySelector(".white-bun");
const addBlackBuns = document.querySelector(".black-bun");
const ing = document.querySelector(".ing");
const beefPatty = document.querySelector("#beef-patty");
const otherPatty = document.querySelector("#other-patty");
const somePatty = document.querySelector("#some-patty");
const tomato = document.querySelector("#tomato");
const lettuce = document.querySelector("#lettuce");
const onion = document.querySelector("#onion");
const pickles = document.querySelector("#pickles");
const normalCheese = document.querySelector("#normal-cheese");
const otherCheese = document.querySelector("#other-cheese");
const ketchup = document.querySelector("#ketchup");
const bbq = document.querySelector("#bbq");
const garlic = document.querySelector("#garlic");
const bun1 = document.querySelector(".bun1");
const bun2 = document.querySelector(".bun2");
const buy = document.querySelector("#buy");
const price = document.querySelectorAll(".price")
let total = document.querySelector("#total");
const buns = document.querySelectorAll(".buns");
let amount = document.querySelector("#amount");
const plus = document.querySelector("#plus");
const minus = document.querySelector("#minus");

let totalValue = 0;
total.textContent = totalValue

let amountValue = 1;
amount.textContent = amountValue

plus.addEventListener("click", () => {
    amountValue += 1
    amount.textContent = amountValue
    total.textContent = +totalValue * +amount.textContent
})

minus.addEventListener("click", () => {
    if (amount.textContent >= 2) {
        amountValue -= 1
        amount.textContent = amountValue
        total.textContent = +totalValue * +amount.textContent
    }
})


const srcs = {
    buns: {
        whiteBuns: {
            topBun: "https://yimages360.s3.amazonaws.com/products/2017/06/593e8c0640e75/1x.jpg",
            bottomBun: "http://atlas-content-cdn.pixelsquid.com/stock-images/hamburger-bun-bottom-koZ0W33-600.jpg"
        },
        blackBuns: {
            topBun: "data:image/jpeg;base64,",
            bottomBun: "data:image/jpeg;base64,"
        }
    },
    patties: {
        beefPatty: "https://yimages360.s3.amazonaws.com/products/2019/08/5d5f9fe478a21/1x.jpg",
        otherPatty: "data:image/jpeg;base64,",
        somePatty: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSpnhoEG-StX84hxl4hn8P62wnqCEnaBplJ4w&usqp=CAU"
    },
    vegetables: {
        tomatoes: "data:image/jpeg;base64,",
        lettuce: "https://i.pinimg.com/736x/28/f2/b2/28f2b26dc6d06373785e1a920177541d.jpg",
        onions: "https://thumbs.dreamstime.com/b/red-onion-slice-side-view-isolated-white-background-clipping-path-onion-slice-isolated-120775401.jpg",
        pickles: "https://media.istockphoto.com/photos/green-pickles-isolated-picture-id1214053645?k=20&m=1214053645&s=612x612&w=0&h=EiUCPk6-aJYXuzblrw0qKRJgVzPOu-lYB4_7wzI9eYU="
    },
    cheese: {
        normalCheese: "https://www.nicepng.com/png/detail/238-2384209_cheese-slice-png.png",
        otherCheese: "data:image/jpeg;base64,"
    },
    sauce: {
        ketchup: "https://atlas-content-cdn.pixelsquid.com/stock-images/ketchup-sauce-cup-VayzrBD-600.jpg",
        bbq: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhwB6K3ebp8ZLzTO_pSwX_-IlM-nhWvOuAhA&usqp=CAU",
        garlic: "https://media.istockphoto.com/photos/small-pot-of-garlic-mayonnaise-isolated-detail-from-above-picture-id475281732?k=20&m=475281732&s=612x612&w=0&h=L5aoj6OgfHNgfAKCv9OmGeIHH8pIGw9dB4rFd9dbJHc="
    }
}

const prices = {
    buns: {
        whiteBuns: 5,
        blackBuns: 5
    },
    patties: {
        beefPatty: 15,
        otherPatty: 20,
        somePatty: 25
    },
    vegetables: {
        tomatoes: 5,
        lettuce: 7,
        onions: 4,
        pickles: 10
    },
    cheese: {
        normalCheese: 10,
        otherCheese: 10
    },
    sauce: {
        ketchup: 5,
        bbq: 5,
        garlic: 5
    }
}

const showSquare = (squareContent) => {
    square.forEach(item => {
        item.style.display = "none"
    })
    squareContent.style.display = "block";
}

const addBuns = (src, secondSrc, price) => {
    ing.style.display = "block"
    topBun.setAttribute("src", src);
    bottomBun.setAttribute("src", secondSrc);
    if (total.textContent === "0") {
        totalValue += 5
        total.textContent = +totalValue
    }
}

const ingredients = (btn, src, price) => {
    btn.addEventListener("click", () => {
        const imgs = document.createElement("img");
        ing.append(imgs)
        imgs.classList.add("buns")
        imgs.setAttribute("src", src);
        imgs.addEventListener("click", () => {
            imgs.style.display = "none"
            totalValue -= price
            total.textContent = +totalValue
        });
        totalValue += price
        total.textContent = +totalValue
    })
}

const imgPrice = (img, src, price) => {
    const imgSrc = img.getAttribute("src");
    if(imgSrc === src){
        totalValue += +price
        total.textContent = +totalValue
    }
}

const minusPrices = (img, src, price) => {
    const imgSrc = img.getAttribute("src");
    if(imgSrc === src){
        totalValue -= +price
        total.textContent = +totalValue
    }
}

images.forEach(img => {
    img.addEventListener("click", () => {
        const imgs = document.createElement("img");
        ing.append(imgs)
        imgs.classList.add("buns");
        const src = img.getAttribute("src");
        imgs.setAttribute("src", src);
            imgPrice(imgs, srcs.patties.beefPatty, prices.patties.beefPatty);
            imgPrice(imgs, srcs.patties.somePatty, prices.patties.somePatty);
            imgPrice(imgs, srcs.patties.otherPatty, prices.patties.otherPatty);
            imgPrice(imgs, srcs.vegetables.tomatoes, prices.vegetables.tomatoes);
            imgPrice(imgs, srcs.vegetables.lettuce, prices.vegetables.lettuce);
            imgPrice(imgs, srcs.vegetables.pickles, prices.vegetables.pickles);
            imgPrice(imgs, srcs.vegetables.onions, prices.vegetables.onions);
            imgPrice(imgs, srcs.cheese.normalCheese, prices.cheese.normalCheese);
            imgPrice(imgs, srcs.cheese.otherCheese, prices.cheese.otherCheese);
            imgPrice(imgs, srcs.sauce.ketchup, prices.sauce.ketchup);
            imgPrice(imgs, srcs.sauce.bbq, prices.sauce.bbq);
            imgPrice(imgs, srcs.sauce.garlic, prices.sauce.garlic);
        imgs.addEventListener("click", () => {
            imgs.style.display = "none"
            minusPrices(imgs, srcs.patties.beefPatty, prices.patties.beefPatty);
            minusPrices(imgs, srcs.patties.somePatty, prices.patties.somePatty);
            minusPrices(imgs, srcs.patties.otherPatty, prices.patties.otherPatty);
            minusPrices(imgs, srcs.vegetables.tomatoes, prices.vegetables.tomatoes);
            minusPrices(imgs, srcs.vegetables.lettuce, prices.vegetables.lettuce);
            minusPrices(imgs, srcs.vegetables.pickles, prices.vegetables.pickles);
            minusPrices(imgs, srcs.vegetables.onions, prices.vegetables.onions);
            minusPrices(imgs, srcs.cheese.normalCheese, prices.cheese.normalCheese);
            minusPrices(imgs, srcs.cheese.otherCheese, prices.cheese.otherCheese);
            minusPrices(imgs, srcs.sauce.ketchup, prices.sauce.ketchup);
            minusPrices(imgs, srcs.sauce.bbq, prices.sauce.bbq);
            minusPrices(imgs, srcs.sauce.garlic, prices.sauce.garlic);
        });
    })
})

addWhiteBuns.addEventListener("click", () => addBuns(srcs.buns.whiteBuns.topBun, srcs.buns.whiteBuns.bottomBun, prices.buns.whiteBuns))
addBlackBuns.addEventListener("click", () => addBuns(srcs.buns.blackBuns.topBun, srcs.buns.blackBuns.bottomBun, prices.buns.blackBuns));

bun1.addEventListener("click", () => addBuns(srcs.buns.whiteBuns.topBun, srcs.buns.whiteBuns.bottomBun, prices.buns.whiteBuns));
bun2.addEventListener("click", () => addBuns(srcs.buns.blackBuns.topBun, srcs.buns.blackBuns.bottomBun, prices.buns.blackBuns));

ingredients(beefPatty, srcs.patties.beefPatty, prices.patties.beefPatty);
ingredients(otherPatty, srcs.patties.otherPatty, prices.patties.otherPatty);
ingredients(somePatty, srcs.patties.somePatty, prices.patties.somePatty);
ingredients(tomato, srcs.vegetables.tomatoes, prices.vegetables.tomatoes);
ingredients(lettuce, srcs.vegetables.lettuce, prices.vegetables.lettuce);
ingredients(onion, srcs.vegetables.onions, prices.vegetables.onions);
ingredients(pickles, srcs.vegetables.pickles, prices.vegetables.pickles);
ingredients(normalCheese, srcs.cheese.normalCheese, prices.cheese.normalCheese);
ingredients(otherCheese, srcs.cheese.otherCheese, prices.cheese.otherCheese);
ingredients(ketchup, srcs.sauce.ketchup, prices.sauce.ketchup);
ingredients(bbq, srcs.sauce.bbq, prices.sauce.bbq);
ingredients(garlic, srcs.sauce.garlic, prices.sauce.garlic);

bun.addEventListener("click", () => showSquare(bunSquare));
patty.addEventListener("click", () => showSquare(pattySquare));
vegetables.addEventListener("click", () => showSquare(vegetablesSquare));
cheese.addEventListener("click", () => showSquare(cheeseSquare));
sauce.addEventListener("click", () => showSquare(sauceSquare));

buy.addEventListener("click", () => {
    alert(` Your Burger Costs ${+total.textContent} lei`);
})
